...
#когда пользователь нажимает на кнопку, включается спиннер на 5 секунд, потом летят шарики
if button:
    ...
    if start_date < end_date:
        ...
        for city in range(len(itog)):
            with st.container():
                ...
                st.image('лого с зонтиком черный.png',width=300)
        #в конце отметим города, для которых не получилось построить прогноз
        for i in no_weather:
    ...
    #выведем предупреждение, если дата конца путешествия раньше, чем начала
    else:
